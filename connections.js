...
var url_login = 'https://sso.pokemon.com/sso/login?service=https%3A%2F%2Fsso.pokemon.com%2Fsso%2Foauth2.0%2FcallbackAuthorize';
var oauth_login = 'https://sso.pokemon.com/sso/oauth2.0/accessToken';

// Google auth
var device_id = '9774d56d682e549c';
...
module.exports = {
  PokemonClub: function (user, pass, self, callback) {
    ...
    self.request.get(options, function (err, response, body) {
      ...
      if (body.trim().indexOf('<') === 0) {
        console.error('Pokemon servers are offline'.red);
        return callback(new Error(('CAS ERROR'), null).red);
      }
      ...
      self.request.post(options, function (err, response, body) {
        ...
        if (body) {
          ...
          if (parsedBody.errors && parsedBody.errors.length !== 0) {
            return callback(new Error(('Login error: ' + parsedBody.errors[0]), null).red);
          }
        }
        ...
        self.request.post(options, function (err, response, body) {
          ...
          if (!token) return callback(new Error(('Login failed'), null).red);
          token = token.split('&')[0];

          if (!token) {
            return callback(new Error('Login failed'.red), null);
          }

          self.DebugPrint(('Session token: ' + token).cyan);
          callback(null, token);
        });
  ...
  GoogleAccount: function (user, pass, self, callback) {
    self.google.login(user, pass, device_id, function (err, data) {
      if (data) {
