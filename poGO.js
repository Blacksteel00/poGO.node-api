...
function Pokeio() {
  ...
  self.GetAccessToken = function (user, pass, callback) {
    ...
    if (self.playerInfo.provider === 'ptc') {
      Connections.PokemonClub(user, pass, self, function (err, token) {
        if (err) {
          return callback(err);
        }
      ...
      Connections.GoogleAccount(user, pass, self, function (err, token) {
        if (err) {
          return callback(err);
        }
